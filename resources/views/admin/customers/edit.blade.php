...
<div class="mb-10">
    ...
    <h2 class="text-3xl font-bold tracking-tight text-slate-800">Edit Data <span class="gradient-text">Pelanggan</span></h2>
    <p class="text-slate-500 mt-1">Perbarui informasi data pelanggan setia LaundryLux.</p>
</div>
        ...
        <form action="{{ route('customers.update', $customer->id) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea id="address" name="address" rows="3"
                          class="block w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-semibold text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:bg-white transition-all"
                          placeholder="Masukkan alamat lengkap rumah pelanggan" required>{{ old('address', $customer->address) }}</textarea>
            </div>
